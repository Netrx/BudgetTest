...
export function init(storage) {
    ...
    migrateExistingDebts(); // Проверяем и сбрасываем долги при загрузке
...
function updateDebtStatuses() {
    ...
    debts.forEach(debt => {
        ...
        if (debt.repeatEnabled && debt.repeatType !== 'none' && isPaid) {
            const paymentDateStr = debt.dueDate; // Используем dueDate как дату оплаты
            
            if (paymentDateStr) {
                const paymentDate = new Date(paymentDateStr);
                paymentDate.setHours(0, 0, 0, 0);
                
                // ===== ИСПРАВЛЕНИЕ: Сбрасываем ТОЛЬКО если сегодня ПОСЛЕ даты оплаты (строго >) =====
...
function migrateExistingDebts() {
    ...
    debts.forEach(debt => {
        if (debt.repeatEnabled && debt.repeatType !== 'none' && (debt.paidAmount || 0) >= debt.amount) {
            if (!debt.dueDate) {
                debt.dueDate = new Date().toISOString().split('T')[0];
                needsSave = true;
            }
        }
...
function renderDebts() {
    ...
    container.innerHTML = filtered.map(debt => {
        ...
        const status = isPaid ? 'paid' : (isOverdue ? 'overdue' : (debt.paidAmount > 0 ? 'partial' : 'active'));
        const statusLabels = {
            paid: 'Погашен',
            partial: 'Частично',
            active: 'Активен',
        ...
        return `
            <div class="debt-card" data-debt-id="${debt.id}" style="${isOverdue ? 'border-color: #EF4444; box-shadow: 0 0 10px rgba(239, 68, 68, 0.1);' : ''}">
...
function openPayDebtModal(id, mode = 'full') {
    ...
    `, (formData) => {
        ...
        // Если долг полностью оплачен, сохраняем дату последней оплаты (на всякий случай)
        if ((debts[index].paidAmount || 0) >= debts[index].amount) {
